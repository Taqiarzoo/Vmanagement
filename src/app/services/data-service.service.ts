...
export class DataServiceService {
 ...
 aadhar: Number=0;
  ...
  loadDetailes(aadhar:number){
    ...
    (
      environment.firebase.userDataBaseURL+aadhar+'.json', 
    )
  }
  //First Vaccine Shoot Data Changes Update on server
  updateUser(element:person,aadharNo:number){
    ...
    (
      environment.firebase.userDataBaseURL+aadharNo+'.json',
      {
          ...
          firstShot: element.firstShot,
          secondShot: element.secondShot,
  ...
  updateUserSecondShoot(secondShoot:boolean,vactnationStatus: boolean, aadharNo:number){
    ...
    (
      environment.firebase.userDataBaseURL+aadharNo+'.json',
      {
          secondShot: secondShoot,
          vactnationStatus: vactnationStatus
 ...
 //#region 
  ...
  check(){
    console.log("Total Names Are: "+this.names.length);
    this.saveUser();
    
    this.savaDataPost(this.personElement).subscribe(response=>{
      console.log("Data Save sussfully");
      console.log(response);
    },error=>{
      console.log("Unknown Error Occure"+error);
    });
  ...
  saveUser(){
    ...
    let obj:person=new person(null,null,null,null,null,null,null,null,null,null,null);
    console.log("function running");  
    for(let i=1;i<=2730;i++){
      let newobj:person=Object.assign({},obj);
      newobj.aadharNo=this.aadhar1;
      this.aadhar1=this.aadhar1+1;
      newobj.name=this.names[i];
      //console.log(newobj.name);
      let age=Math.floor(Math.random()*80)+1;
      newobj.age=age;
      if(newobj.age>20){
        newobj.eligibility=true;
      }else{
        newobj.eligibility=false;
      }
      newobj.vactnationStatus=false;
        newobj.firstdose=new Date();
        newobj.seconddose=new Date();
        newobj.firstShot=false;
        newobj.secondShot=false;
        newobj.isSecondShot=false;
        newobj.perBook=new Date(2999,10,10);
        newobj.isPreBook=false;
        newobj.vaccine="No VAccine yet";
        this.personElement.push(newobj);
        
    }
    
  }
  arrayToJason(array:any){
    return JSON.parse( JSON.stringify( array ) );
  }
  savaDataPost(persons:any){
    let result:person = persons.reduce((obj, cur) => ({...obj, [cur.aadharNo]: cur}), {})
    console.log(result);
    // let persons:person=new person(null,null,null,null,null,null,null,null,null,null,null);
    return this.httpClient
    .post<person>(
      'https://vmanagement-22ebf-default-rtdb.firebaseio.com/User.json',
      result
    ).pipe(map(data=>{
  ...
  deleteDatabase(){
    this.deteteData().subscribe(desponse=>{
